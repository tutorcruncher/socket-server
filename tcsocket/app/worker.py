...
async def get_image(ctx, company, contractor_id, url):
    save_dir = ctx['media'] / company
...
async def update_contractors(ctx, company, app):
...
async def post_data(ctx, url, data, company):
    ...
    headers = ctx['request_headers'](company, {'Content-Type': CT_JSON})
...
async def submit_enquiry(ctx, company, data):
    api_enquiries_url = ctx['settings'].api_root + ctx['settings'].tc_enquiry_endpoint
...
async def submit_booking(ctx, company, data):
    api_book_appointment_url = ctx['settings'].api_root + ctx['settings'].settings.tc_book_apt_endpoint
...
class WorkerSettings:
    functions = [get_image, update_contractors, update_enquiry_options, submit_enquiry, submit_booking]
