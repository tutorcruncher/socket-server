...
async def test_service_filter(cli, db_conn, company):
    ...
    await create_appointment(
        db_conn,
        company,
        appointment_extra={"id": 1, "start": midnight + timedelta(seconds=3)},
    )
    ...
    await create_appointment(
        db_conn,
        company,
        appointment_extra={"id": 4, "start": midnight - timedelta(seconds=1)},
        service_extra={"id": 3},
    )
...
async def test_service_list(cli, db_conn, company):
    ...
    await create_appointment(
        db_conn,
        company,
        appointment_extra={"id": 3, "start": datetime(1986, 1, 1)},
        create_service=False,
    )
    await create_appointment(
        db_conn,
    ...
    await create_appointment(
        db_conn,
        company,
        appointment_extra={"id": 5, "start": datetime(1986, 1, 1)},
        service_extra={"id": 3},
    )
...
async def test_check_client_data(cli, company, db_conn):
    await create_appointment(
        db_conn,
        company,
        appointment_extra={"id": 42, "attendees_current_ids": [384924]},
    )
    await create_appointment(
        db_conn,
        company,
...
async def test_submit_appointment(cli, company, appointment, other_server, worker):
    ...
    r = await cli.post(
        url,
        data=json.dumps({"appointment": appointment["appointment"]["id"], "student_id": "4"}),
    )
...
async def test_submit_double_book(cli, company, appointment, other_server):
    ...
    r = await cli.post(
        url,
        data=json.dumps({"appointment": appointment["appointment"]["id"], "student_id": "3"}),
    )
    assert r.status == 400, await r.text()
...
async def test_book_appointment_options_handler(cli, company):
    ...
    url = cli.server.app.router['book-appointment-options'].url_for(company='thepublickey')
    r = await cli.request('OPTIONS', url)
    assert r.status == 200, await r.text()
